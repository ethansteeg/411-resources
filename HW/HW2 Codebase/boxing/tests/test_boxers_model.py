...
def normalize_whitespace(sql_query: str) -> str:
    """Normalizes whitespace in a SQL query for comparison."""
    return re.sub(r"\s+", " ", sql_query).strip()

@pytest.fixture
def mock_cursor(mocker):
    """Mocks the database cursor and connection for testing."""
...
def test_create_boxer_valid(mock_cursor):
    """Tests creating a boxer with valid inputs."""
...
def test_create_boxer_sql_error(mock_cursor):
    """Tests creating a boxer when a database error occurs."""
...
def test_delete_boxer_valid(mock_cursor):
    """Tests deleting a boxer with a valid ID."""
    mock_cursor.fetchone.return_value = (1,)
    delete_boxer(None, 1)
...
def test_get_boxer_by_id_valid(mock_cursor):
    """Tests getting a boxer by a valid ID."""
...
def test_get_boxer_by_id_not_found(mock_cursor):
    """Tests getting a boxer by an ID that doesn't exist."""
    mock_cursor.fetchone.return_value = None
    with pytest.raises(ValueError, match="Boxer with ID 999 not found"):
        get_boxer_by_id(999)

def test_get_boxer_by_id_invalid():
    """Tests getting a boxer by an invalid ID (negative)."""
    with pytest.raises(ValueError, match="Invalid boxer ID: -1"):
        get_boxer_by_id(-1)

def test_get_boxer_by_id_db_error(mock_cursor):
    """Tests getting a boxer by ID when a database error occurs."""
    mock_cursor.execute.side_effect = sqlite3.Error("DB fail")
    with pytest.raises(sqlite3.Error):
        get_boxer_by_id(1)

def test_get_boxer_by_name_valid(mock_cursor):
    """Tests getting a boxer by a valid name."""
...
def test_get_boxer_by_name_invalid():
    """Tests getting a boxer by an invalid name (empty string)."""
    with pytest.raises(ValueError, match="Invalid boxer name: "):
        get_boxer_by_name("")

def test_get_boxer_by_name_not_found(mock_cursor):
    """Tests getting a boxer by a name that doesn't exist."""
    mock_cursor.fetchone.return_value = None
...
def test_get_boxer_by_name_db_error(mock_cursor):
    """Tests getting a boxer by name when a database error occurs."""
...
def test_get_weight_class_valid():
    """Tests getting the weight class for valid weights."""
...
def test_get_weight_class_invalid():
    """Tests getting the weight class for an invalid weight."""
